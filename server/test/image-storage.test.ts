...
test('importImageFile never uses the caller-provided name for the on-disk filename', async () => {
  ...
  assert.match(output.fileName, /^job-x-[0-9a-f]{12}\.png$/)
})
...
test('deleteImageFile removes an existing file', async () => {
  ...
  await deleteImageFile('img-test-8', output.relativePath)
...
test('deleteImageFile does not throw when the file is already missing', async () => {
  await assert.doesNotReject(() => deleteImageFile('img-test-9', 'assets/images/imported/already-gone.png'))
})

test('deleteImageFile rejects a relativePath that escapes the project image directory', async () => {
  await assert.rejects(() => deleteImageFile('img-test-10', '../../../etc/passwd'), PathEscapeError)
})
